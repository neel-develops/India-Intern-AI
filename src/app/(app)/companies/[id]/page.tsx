        ...
        <Card className="overflow-hidden">
            <div className="bg-muted/30 p-8">
                <div className="flex flex-col sm:flex-row items-start gap-6">
                    <Image src={company.logo} alt={`${company.name} logo`} width={128} height={128} className="rounded-lg border-4 border-background shadow-lg" data-ai-hint={`${company.name}`} />
                    <div className="space-y-2">
                        <h1 className="text-4xl font-bold tracking-tight">{company.name}</h1>
                        <Link href={company.website} target="_blank" className="inline-flex items-center gap-2 text-muted-foreground hover:text-primary">
